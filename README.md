...
- **Delivery session.** Push is handled by a singleton delivery session spawned per topic on first click. It runs directly on the topic branch (not a worktree) so pushes, PR edits, CI investigation, and comment triage all happen in one place that can see the real branch. Push only appears when the topic is in a clean state (every task accepted or discarded, no uncommitted changes anywhere, topic branch ahead of main). Deleting the topic reaps the delivery session too.
