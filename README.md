...
- **Topic dashboard.** Sessions are grouped into topics — one card per work item, sorted by activity. Create topics from the "+ New topic" modal: pick a template (Quick fix / Standard / Exploration), select a repo, set a title, optional ticket key. Creating a topic cuts the branch but does not spawn any session; click into the topic and press "+ New attempt" to start work with your chosen prompt, effort, and permission mode.
