...
- **Deterministic git ops + Claude-written text.** Save, Merge, Push, and Create PR are server-side git/gh commands — never asked of a live Claude session — so they're fast (seconds), reproducible, and auditable. Claude is called once per operation via `claude -p --max-turns 1` to generate only the *text* of the commit message / PR title / PR body, using the repo's own skills, CLAUDE.md, AGENTS.md, and PR template as context. Merge runs in an isolated temp worktree on the topic branch, so the main clone's state is never touched. No more delivery session, no main-clone branch-switch dance, no stale index.lock races.
