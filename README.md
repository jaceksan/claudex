...
- **Topic dashboard (preview).** Sessions are grouped into topics — one card per work item, sorted by activity. Create topics from the "+ New topic" modal: pick a template (Quick fix / Standard / Exploration), select a repo, set a title, optional ticket key, initial prompt, effort level, and permission mode. Template choice auto-sets sensible defaults for effort and permission. Topic detail lands in the next plan.
