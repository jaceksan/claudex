...
- **Topic detail page.** Three-column layout: timeline stepper (Draft → PR opened → Under review → Merge), task list with per-task **Save** (commit uncommitted work in the worktree), **Merge to topic** (squash-merge into the topic branch — refuses if the worktree is dirty), **Discard changes** (drop uncommitted, keep commits), **Discard task** (kill session + remove worktree + delete branch). Review-comment cards with per-thread Fix and inline reply, CI check panel with rollup badge, a header status dot (amber pulse while running → green/red once done), and per-check Fix. Sticky action bar gates buttons by phase — Create PR (Draft, accepted task), Address feedback (Open, unresolved comments or failing CI), plus a **Push** button that publishes the topic branch to the fork remote. No local Merge button: in almost every team with compliance, the PR author can't merge their own PR.
...
- **Rule-aware buttons.** Save, Merge to topic, and Push don't run raw `git` on the server — they inject a prompt into the matching Claude session so the repo's own commit-message / merge / push rules (internal skills, plugin skills, CLAUDE.md, MCP — in that priority order) are honoured. Buttons hide while Claude is mid-turn to avoid queuing behind a running task. Discard changes and Discard task stay direct (they destroy, nothing for Claude to "do right").
- **Delivery session.** Push and Create PR are both handled by a singleton delivery session spawned per topic on first click. It runs directly on the topic branch (not a worktree) so pushes, PR creation + edits, CI investigation, and comment triage all happen in one place that can see the real branch. Push only appears when the topic is in a clean state (every task accepted or discarded, no uncommitted changes anywhere, topic branch ahead of main). Create PR injects an "open PR per repo conventions" prompt — Claude writes the title and body using the repo's CLAUDE.md / skills / MCP rules (the old hard-coded `_TODO_` template body is gone). The server polls `gh pr list` after the prompt to capture the new PR number and auto-arm CI watching. Deleting the topic reaps the delivery session too.
